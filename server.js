...
function requireUser(req,res,next) {
  ...
  req.tgUser = tgUser; req.user = upsertUser(tgUser); next();
}
...
app.get('/api/me', requireUser, (req,res) => {
  ...
  res.json({ user:req.user, credits:c?.balance || 0, freeAvailable: Math.max(0,req.user.free_generations_limit-req.user.free_generations_used), history, purchases });
});
...
app.post('/api/generations', requireUser, (req,res) => {
  ...
  const tx = db.transaction(() => {
    if (useFree) db.prepare('UPDATE users SET free_generations_used=free_generations_used+1,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(req.user.id);
    else db.prepare('UPDATE credits SET balance=balance-?,updated_at=CURRENT_TIMESTAMP WHERE user_id=?').run(style.price_credits,req.user.id);
    const result = db.prepare('INSERT INTO generations (user_id,showcase_style_id,status,source_image,credits_spent) VALUES (?,?,?,?,?)').run(req.user.id,style.id,'queued',sourceImage || null,useFree ? 0 : style.price_credits);
    return result.lastInsertRowid;
  });
  const generationId = tx();
  res.json({ok:true,generationId,chargedFree:useFree,creditsSpent:useFree?0:style.price_credits,message:'Генерация поставлена в очередь. AI-провайдер подключим следующим модулем.'});
});
...
app.post('/api/credits/topup', requireUser, (req,res) => {
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(`INSERT INTO transactions (user_id,type,amount,credits,status,provider,description) VALUES (?,?,?,?,?,?,?)`).run(req.user.id,'credit',0,credits,'demo','demo','Демо-пополнение; заменить на Telegram Stars/платёжный провайдер');
  });
  tx();
  res.json({ok:true});
});

const upload = multer({ storage: multer.diskStorage({ destination:UPLOAD_DIR, filename:(_,file,cb)=>cb(null,`${Date.now()}-${crypto.randomUUID()}${path.extname(file.originalname)}`) }), limits:{fileSize:10*1024*1024} });
...
app.get('/api/admin/showcase', requireAdmin, (req,res)=>res.json(db.prepare('SELECT * FROM showcase_styles ORDER BY sort_order,id').all().map(x=>({...x,preview_images:JSON.parse(x.preview_images)}))));
app.post('/api/admin/showcase', requireAdmin, upload.array('images',8), (req,res) => {
  ...
  const result=db.prepare(`INSERT INTO showcase_styles (title,category,description,price_credits,is_popular,sort_order,preview_images) VALUES (?,?,?,?,?,?,?)`).run(title,category,description,Number(priceCredits),Number(isPopular),Number(sortOrder),JSON.stringify(images));
