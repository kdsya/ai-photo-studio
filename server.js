...
async function ensureUser(tgUser) {
  ...
  if (Object.prototype.hasOwnProperty.call(tgUser, 'photo_url')) {
    patch.photo_url = tgUser.photo_url || null;
  }
...
async function requireUser(req, res, next) {
  try {
    ...
    req.isAdmin = userId === adminId || userUsername === adminName;

    console.log(`User ${userId} (${userUsername}) isAdmin: ${req.isAdmin}`); // Для отладки
...
async function requireAdmin(req, res, next) {
  await requireUser(req, res, () => req.isAdmin ? next() : res.status(403).json({ error: 'Admin access denied' }));
}
...
app.get('/api/me', requireUser, async (req, res) => {
  try {
    ...
    console.log('Sending /api/me response for user:', req.user.id, 'isAdmin:', response.isAdmin);
    res.json(response);
  } catch (e) {
...
app.get('/api/showcase/:category', async (req, res) => {
...
app.get('/api/packages', async (req, res) => res.json({
...
app.post('/api/generations', requireUser, upload.fields([
  ...
  try {
    ...
    let price = useFree ? 0 : Number(style.price_credits || 40);
    ...
    if (useFree) {
      ...
      const { data: claimed, error: claimError } = await supabase
        .rpc('claim_free_generation', { p_user_id: req.user.id });
      ...
      try {
        ...
        return res.json({
          ok: true,
          message: 'Бесплатная генерация принята. AI-воркер подключим следующим этапом.',
          creditsSpent: 0,
...
app.post('/api/admin/packs', requireAdmin, async (req, res) => {
  ...
  if (!['him', 'her', 'couple'].includes(category)) {
    return res.status(400).json({ error: 'Неверная категория' });
  }

  if (!title || !image_url) {
    return res.status(400).json({ error: 'Название и ссылка на изображение обязательны' });
  }

  if (list.length !== 40) {
    return res.status(400).json({ error: `Нужно ровно 40 промтов. Сейчас: ${list.length}` });
  }
...
app.patch('/api/admin/packs/:id', requireAdmin, async (req, res) => {
  ...
  if (u.prompts) {
    if (!Array.isArray(u.prompts) || u.prompts.length !== 40) {
      return res.status(400).json({ error: 'Нужно ровно 40 промтов' });
    }
...
app.patch('/api/admin/free', requireAdmin, async (req, res) => {
  ...
  if (!Number.isInteger(limit) || limit !== 1) {
    return res.status(400).json({ error: 'В системе доступна ровно 1 бесплатная генерация на пользователя' });
  }
