...
app.get('/api/home',async(req,res)=>{try{const {data:cards,error}=await supabase.from('hero_cards').select('key,title,description,image_url,sort_order').eq('is_active',1).order('sort_order');if(error)throw error;res.set('Cache-Control','public, max-age=30, stale-while-revalidate=120');res.json(cards?.length?cards:DEFAULT_HERO)}catch(e){console.error('home:',e);res.status(500).json({error:'Не удалось загрузить главные карточки'})}});
app.get('/api/showcase',async(req,res)=>{const {data,error}=await supabase.from('showcase_styles').select('id,category,title,description,image_url,preview_images,price_credits,is_popular,is_active,sort_order').eq('is_active',1).order('sort_order');if(error)return res.status(500).json({error:error.message});res.set('Cache-Control','public, max-age=30, stale-while-revalidate=120');res.json(data||[])});
app.get('/api/showcase/:id',async(req,res)=>{const {data,error}=await supabase.from('showcase_styles').select('*').eq('id',Number(req.params.id)).eq('is_active',1).maybeSingle();if(error)return res.status(500).json({error:error.message});if(!data)return res.status(404).json({error:'Фотосессия не найдена'});res.json(data)});
...
app.post('/api/generations',requireUser,upload.fields([{name:'sourceImage1',maxCount:1},{name:'sourceImage2',maxCount:1}]),async(req,res)=>{try{const {showcaseStyleId,category,free}=req.body;const useFree=free==='1';let style=null;if(!useFree){const r=await supabase.from('showcase_styles').select('*').eq('id',Number(showcaseStyleId)).eq('is_active',1).maybeSingle();style=r.data;if(!style)return res.status(400).json({error:'Фотосессия не найдена'});if(style.category!==category)return res.status(400).json({error:'Категория не совпадает'});if(!Array.isArray(style.prompts)||style.prompts.length!==40)return res.status(400).json({error:'У пака должно быть 40 промтов'})}
 ...
 if(useFree){
   // Атомарно забираем единственную бесплатную генерацию в БД.
   // Это защищает от двойных/десятикратных быстрых кликов и параллельных запросов.
   const {data:claimed,error:claimError}=await supabase.rpc('claim_free_generation',{p_user_id:req.user.id});
   if(claimError){console.error('claim_free_generation:',claimError);return res.status(500).json({error:'Не удалось проверить бесплатную генерацию'});}
   ...
   try {
     ...
     // Если запись генерации не создалась, возвращаем право на бесплатную генерацию.
...
app.patch('/api/admin/free',requireAdmin,async(req,res)=>{const prompt=String(req.body.prompt||'').trim();const limit=Number(req.body.limit);if(!prompt)return res.status(400).json({error:'Промт не может быть пустым'});if(!Number.isInteger(limit)||limit!==1)return res.status(400).json({error:'В системе доступна ровно 1 бесплатная генерация на пользователя'});await saveSetting('free_generation_prompt',prompt);await saveSetting('free_generation_limit','1');await supabase.from('users').update({free_generations_limit:1});res.json({ok:true})});
