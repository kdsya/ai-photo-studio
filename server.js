...
// === НАСТРОЙКА АДМИНА ===
const ADMIN_ID = process.env.ADMIN_TELEPHONE_ID || '6711149865';
const ADMIN_USERNAME = process.env.ADMIN_TELEGRAM_USERNAME || 'tgfsb';
...
app.post('/api/generations', requireUser, (req,res) => {
  ...
  res.json({ok:true, generationId, chargedFree:useFree, creditsSpent:useFree?0:style.price_credits, message:'Генерация поставлена в очередь'});
});
...
app.post('/api/credits/topup', requireUser, (req,res) => {
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(`INSERT INTO transactions (user_id,type,amount,credits,status,provider,description) VALUES (?,?,?,?,?,?,?)`).run(req.user.id, 'credit', 0, credits, 'demo', 'demo', 'Демо-пополнение');
  });
  tx();
  res.json({ok:true});
});
